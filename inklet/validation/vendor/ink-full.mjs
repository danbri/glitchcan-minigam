...
class InkList extends Map {
  ...
  AddItem(itemOrItemName) {
    let storyObject = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : null;
    if (itemOrItemName instanceof InkListItem) {
      ...
      if (foundListDef == null) {
        if (storyObject == null) {
          ...
          let newItem = InkList.FromString(itemName, storyObject).orderedItems[0];
...
class Container extends InkObject {
  ...
  ContentAtPath(path) {
    ...
    for (let i = partialPathStart; i < partialPathLength; ++i) {
      ...
      const nextContainer = asOrNull(foundObj, Container);
...
class NativeFunctionCall extends InkObject {
  ...
  Call(parameters) {
    ...
    for (let p of parameters) {
      if (p instanceof Void) throw new StoryException("Attempting to perform " + this.name + ' on a void value. Did you forget to "return" a value from a function you called here?');
...
let Divert$1 = class Divert extends InkObject {
  get targetPath() {
...
let VariableAssignment$1 = class VariableAssignment extends InkObject {
...
let Choice$1 = class Choice extends ParsedObject {
  get runtimeChoice() {
...
let VariableReference$1 = class VariableReference extends InkObject {
...
let ListDefinition$1 = class ListDefinition {
...
let Glue$1 = class Glue extends Wrap {
  constructor(glue) {
    super(glue);
  }
  get typeName() {
...
let Tag$1 = class Tag extends InkObject {
  constructor(tagText) {
    super();
    ...
    if (args !== null) {
      for (let i = 0; i < args.length; i++) {
        if (!(typeof args[i] === "number" || typeof args[i] === "string" || typeof args[i] === "boolean" || args[i] instanceof InkList)) {
          throw new Error("ink arguments when calling EvaluateFunction / ChoosePathStringWithParameters must be" + "number, string, bool or InkList. Argument was " + (nullIfUndefined(args[i]) === null ? "null" : args[i].constructor.name));
        }
...
let Story$1 = class Story extends InkObject {
  get currentChoices() {
    let choices = [];
    if (this._state === null) {
...
InkParser.Latin1Supplement = CharacterRange.Define("\u0080", "\u00FF", new CharacterSet());
InkParser.Chinese = CharacterRange.Define("\u4E00", "\u9FFF", new CharacterSet());
/// <summary>
...
/// non-ASCII character ranges that can be used in identifier names.
/// </returns>
InkParser.ListAllCharacterRanges = () => [InkParser.LatinBasic, InkParser.LatinExtendedA, InkParser.LatinExtendedB, InkParser.Arabic, InkParser.Armenian, InkParser.Cyrillic, InkParser.Greek, InkParser.Hebrew, InkParser.Korean, InkParser.Latin1Supplement, InkParser.Chinese];
...
function GenerateStoryStats(story) {
  ...
  const choices = story.FindAll(Choice$1)();
...
export { Compiler, CompilerOptions, InkList, InkParser, JsonFileHandler, StatementLevel, Story$1 as Story };
