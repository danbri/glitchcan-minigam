...
try {
  ...
  await page.waitForSelector('[data-app="editor"] edot-editor .page');
  ...
  await page.evaluate(() => {
    const ed = document.querySelector('[data-app="editor"] edot-editor');
  ...
  ok('Editor menu action works (View → Heading 1)', /<h1/i.test(await page.evaluate(() => document.querySelector('[data-app="editor"] edot-editor').getContent())));

  // Data app: MS-Access-style data views in the View menu, and an action works.
  await page.click('.rail button[data-nav="data"]');
  await page.waitForSelector('[data-app="data"] edot-data .dw');
  ...
  ok('Data menu action works (load sample DB populates objects)',
    (await page.evaluate(() => document.querySelectorAll('[data-app="data"] .dw-side .dw-item').length)) > 5);
  ...
  ok('View → Open app switches the active app', await page.evaluate(() => location.hash === '#calendar' && !document.querySelector('[data-app="calendar"]').hidden));
