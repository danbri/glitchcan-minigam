...
/**
 * Create a quadruped physics rig
 * Body + 4 feet connected by distance constraints
 *
 * @param {XPBDPhysicsGPU} physics - Physics system
 * @param {Object} config - Quadruped configuration
 * @returns {Object} Particle indices and constraint indices
 */
export function createQuadrupedRig(physics, config = {}) {
  const {
    bodyPos = [0, 0.5, 0],
    bodyMass = 5.0,
    footMass = 0.5,
  ...
  // Body particle (heavy, central mass)
  const bodyIdx = physics.addParticle({
    pos: bodyPos,
    mass: bodyMass,
    radius: 0.3,
    name: 'body'
  ...
  const footOffsets = [
    [bodyWidth, -legLength, bodyLength],   // Front-left
    [-bodyWidth, -legLength, bodyLength],  // Front-right
    [bodyWidth, -legLength, -bodyLength],  // Rear-left
    [-bodyWidth, -legLength, -bodyLength]  // Rear-right
  ];

  const footNames = ['footFL', 'footFR', 'footRL', 'footRR'];
  const footIndices = [];
  ...
  for (let i = 0; i < 4; i++) {
    const footPos = [
      bodyPos[0] + footOffsets[i][0],
      bodyPos[1] + footOffsets[i][1],
      bodyPos[2] + footOffsets[i][2]
    ];

    const footIdx = physics.addParticle({
      pos: footPos,
      mass: footMass,
      radius: 0.1,
      name: footNames[i]
    ...
    footIndices.push(footIdx);

    // Distance constraint: body to foot (leg)
    physics.addDistanceConstraint(bodyIdx, footIdx, legLength, compliance);
    legConstraints.push(physics.distConstraints.length - 1);

    // Position constraint: foot to target (updated by walk cycle)
    physics.addPositionConstraint(footIdx, footPos, 0.01);
    footConstraints.push(physics.posConstraints.length - 1);
  }
