...
window.FinkBreadcrumb = {
    ...
    // Generate navigation URL for a knot
    generateKnotUrl(knotName) {
        if (!this.currentFinkUrl) return '#';

        // Create a shareable URL with story and knot
    ...
    copyKnotUrl(knotName) {
        const url = this.generateKnotUrl(knotName);
        navigator.clipboard.writeText(url).then(() => {
            FinkUI.showStatus('Link copied!');
            setTimeout(() => FinkUI.hideStatus(), 1000);
        }).catch(err => {
    ...
    // Get current knot from INK story state
    // This is a heuristic - INK doesn't directly expose "current knot"
    getCurrentKnotFromStory() {
        if (!FinkInkEngine.story) return null;

        try {
            // Try to get the current path
            const state = FinkInkEngine.story.state;
            if (state && state.currentPath) {
                const pathString = state.currentPath.toString();
            ...
            FinkUtils.debugLog('Breadcrumb: Could not extract current knot: ' + e.message);
        }
